        ...
        .def("solver",
             [](GridBackwardEulerDiffusionSolver2& instance, Grid2Ptr source,
                ...
                Grid2Ptr dest, pybind11::kwargs kwargs) {
                 ...
                 if (kwargs.contains("boundary_sdf"))
                 {
                     boundarySDF = kwargs.cast<ScalarField2Ptr>();
                 }
                 if (kwargs.contains("fluid_sdf"))
                 {
			...
			----------
			- source : Input grid.
			- diffusion_coefficient : Amount of diffusion.
			- time_interval_in_seconds : Small time-interval that diffusion occur.
			- dest : Output grid.
			- `**kwargs` :
				- Key `boundary_sdf` : Shape of the solid boundary that is empty by default.
				- Key `fluid_sdf` : Shape of the fluid boundary that is full by default.
		)pbdoc",
             pybind11::arg("source"), pybind11::arg("diffusion_coefficient"),
             pybind11::arg("time_interval_in_seconds"), pybind11::arg("dest"));
}
