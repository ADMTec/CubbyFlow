...
{
    ParallelFor(ZERO_SIZE, m_dataU.Width(), ZERO_SIZE, m_dataU.Height(),
                [this, value](size_t i, size_t j) { m_dataU(i, j) = value.x; },
                policy);

    ParallelFor(ZERO_SIZE, m_dataV.Width(), ZERO_SIZE, m_dataV.Height(),
                [this, value](size_t i, size_t j) { m_dataV(i, j) = value.y; },
                policy);
}
...
double FaceCenteredGrid2::Divergence(const Vector2D& x) const
{
    ...
    GetBarycentric(normalizedX.x, 0,
                   static_cast<ssize_t>(GetResolution().x) - 1, &i, &fx);
    GetBarycentric(normalizedX.y, 0,
                   static_cast<ssize_t>(GetResolution().y) - 1, &j, &fy);
...
void FaceCenteredGrid2::ResetSampler()
{
    LinearArraySampler2<double, double> uSampler(
        m_dataU.ConstAccessor(), GetGridSpacing(), m_dataOriginU);
    LinearArraySampler2<double, double> vSampler(
        m_dataV.ConstAccessor(), GetGridSpacing(), m_dataOriginV);
